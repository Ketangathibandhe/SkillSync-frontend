...
import {
  ...
  deleteRoadmapById,
  updateStepStatus,
  analyzeSkillGap, // Skill Gap action
} from "../utils/skillSlice";
...
const RoadmapPage = () => {
  ...
  const handleAnalyzeSkillGap = () => {
    if (!targetRoleInput || !currentSkillsInput) return;
    const skillsArray = currentSkillsInput.split(",").map((s) => s.trim());
    dispatch(analyzeSkillGap({ targetRole: targetRoleInput, currentSkills: skillsArray }));
  };
    ...
    <div className="flex flex-col md:flex-row min-h-screen mb-14">
      ...
      <div className="flex-1 p-4 md:p-6 overflow-y-auto">
        ...
        <div className="mb-6 p-4 border rounded bg-yellow-100">
          <h2 className="text-xl font-bold mb-2">Skill Gap Analysis</h2>
          ...
          {loading && <p>Analyzing skill gap...</p>}
          {error && <p className="text-red-500">{error}</p>}
          ...
          {skillGapResult && (
            <div className="mt-4">
              ...
              {skillGapResult.learningPriorities?.length > 0 && (
                <div className="mb-2">
                  <h4 className="font-semibold">Learning Priorities:</h4>
                  <ol className="list-decimal ml-5">
                    {skillGapResult.learningPriorities.map((priority, idx) => (
        ...
        {!loading &&
          userRoadmaps.length === 0 && (
            <div className="flex flex-col items-center justify-center h-full text-gray-600">
              <p className="text-lg font-semibold">No roadmap generated yet</p>
              <p className="text-sm">
                Generate a roadmap after analyzing your skill gap
              </p>
            </div>
          )}
        ...
        {!loading &&
          userRoadmaps.length > 0 &&
          !hasSteps && <p className="text-center">No roadmap selected</p>}
